...
int16_t  SWTFT::getNextCursorY()
{
  return (cursor_y + textsize*8);
}

#if ARDUINO >= 100
size_t SWTFT::write(uint8_t c) {  
#else
void SWTFT::write(uint8_t c) {
#endif
  Adafruit_GFX::write(c);
  if ( getNextCursorY() > _height )
    setCursor(0,0);
#if ARDUINO >= 100
  return 1;
#endif
...
void SWTFT::setColor(uint16_t color)
{
  textcolor = color;
}

void SWTFT::setBGColor(uint16_t bgcolor)
{
  textbgcolor = bgcolor;
}

uint16_t SWTFT::getColor()
{
  return textcolor;
}

uint16_t SWTFT::getBGColor()
{
  return textbgcolor;
}

uint8_t SWTFT::getFontSize()
{
  return textsize * 8;
}

void SWTFT::drawText( char * label, int16_t x, int16_t y )
{
  int16_t pre_cursorX = cursor_x;
  int16_t pre_cursorY = cursor_y;
  setCursor( x, y );
  Adafruit_GFX::print(label);
  setCursor( pre_cursorX, pre_cursorY );
}


void SWTFT::SW_fillRoundRect(int16_t x, int16_t y, uint16_t w, uint16_t h)
{
  Adafruit_GFX::fillRoundRect(x, y, w, h, w/8, textcolor);
}

void SWTFT::SW_drawRoundRect(int16_t x, int16_t y, uint16_t w, uint16_t h)
{
  Adafruit_GFX::drawRoundRect( x, y, w, h, w/8, textcolor);
}
